...
int main(int argc, char *argv[])
{
    ...
    while ((argi <= argc - 1) && (type = getop(s, argv[argi++])))
    {
        switch (type)
        {
        case NUMBER:
...
int getop(char s[], char arg[])
{
    ...
    while ((s[0] = c = arg[i_arg]) == ' ' || c == '\t')
        ;
    s[1] = '\0';

    if (isalpha(c))
    {
        while (isalpha(s[++i] = c = arg[++i_arg]))
            ;
        s[i] = '\0';
        if (i > MATH_S_LEN)
            return STACK_OP;
    ...
    if ((c == '+' || c == '-') && isdigit(c_next = arg[++i_arg]))
    {
        s[1] = c_next;
        c = c_next;
        i = 1;
